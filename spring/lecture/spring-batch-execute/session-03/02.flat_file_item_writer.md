...
- 한 번에 너무 많은 데이터를 처리해야 한다면 문제가 많이 발생할 것이다.
  - 파일이 너무 커서 자원이 과도하게 소모된다.
  - 파일을 열고 읽는데 시간이 너무 오래 걸린다.
  - 백업이나 전송 시 파일 크기 제한이 걸림돌이 된다.
  - 데이터가 너무 커서 분석이 힘들다.
- `MultiResourceItemWriter`
  - 대량의 데이터를 여러 개의 리소스 파일로 분배하는 `ItemWriter` 구현체
  - 파일을 직접 쓰지는 않고 실제로 파일 쓰기를 처리할 ItemWriter에게 작업을 위임한다. (`delegateItemWriter()`)
