...
- `MultiResourceItemReader`
  - 여러 파일을 순차적으로 읽을 수 있게 해주는 `ItemReader` 구현체
  - `MultiResourceItemReader`가 실제로 파일 읽기를 수행하지 않는다.
  - 실제 파일 읽기는 위임 대상 `ItemReader`가 수행한다.
  - `MultiResourceItemReaderBuilder` 전용 빌더를 통해 생성할 수 있다.
...
- 파일 간의 순서 지정이 필요하다면 `comparator`를 활용할 수 있다.
...
- `delegate`로 지정한 위임 대상 `ItemReader`는 다른 파일 읽기 `ItemReader`와 다르게 `resource` 정보를 파라미터로 받지 않는다.
  - `MultiResourceItemReader`가 직접 지정해주기 때문
