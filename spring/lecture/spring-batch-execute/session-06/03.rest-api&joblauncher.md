...
- `stop()` 호출 시점에 제어권은 Spring Batch가 아닌 비즈니스 로직(Tasklet/ItemReader 등)에 있을 수 있다.
- Spring Batch는 의도적으로 **현재 처리 중인 청크를 완료한 후** 중지하도록 설계되었다.
  - 중지 감지는 청크 경계(`doInChunkContext()`)에서 `checkInterrupted()`를 통해 이루어짐
  - 즉, `stop()` 호출 → 현재 청크 완료 → 다음 청크로 넘어가기 전에 중지
- 청크 완료 후 중지하는 이유
  - **데이터 일관성**: 이미 읽고 변환한 데이터를 완전히 처리 완료해야 데이터 일관성을 보장
  - **리소스 효율성**: 이미 처리한 데이터를 버리지 않고 완료하여 계산 리소스 낭비 방지
  - **롤백 불가능한 Writer 대응**: `KafkaItemWriter` 등 롤백이 불가능한 구현체에서는 중간 중단 시 데이터 일관성이 깨질 수 있다.
...
- 전달받은 `executionId`로 `JobExecution`을 조회하고, `JobRegistry`에서 `Job` 빈을 찾아 `JobLauncher.run()`을 호출
- 재시작의 실질적인 로직은 `JobLauncher` 내부에 있으므로 `restart()` 자체는 단순한 위임 역할
...
## 부록: 메시지 기반 Job 실행 - Spring Batch Integration

- Spring Batch Integration은 Spring Batch에 Spring Integration의 메시징 기능을 결합한 확장 모듈
- RabbitMQ, Kafka 등 외부 메시징 시스템으로부터 수신한 메시지를 트리거로 배치 `Job`을 자동 실행할 수 있다.

### JobLaunchingGateway

- Spring Integration 메시지를 받아 배치 `Job`을 실행하는 게이트웨이
- 내부적으로 `JobLaunchingMessageHandler`가 `JobLaunchRequest`에서 `Job`과 `JobParameters`를 추출하여 `JobLauncher.run()`을 호출
...
- 메시지 수신 → `JobLaunchRequest` 변환 → `JobLaunchingGateway`에서 `Job` 실행
- 변환 단계에서 `JobRegistry`와 `JobParametersBuilder`를 사용하여 `JobLaunchRequest`를 생성

```java
// Spring Integration DSL로 정의한 흐름
...
- 진입점만 달라질 뿐, Spring Batch 코어 엔진은 하나다.
