...
- 원격 파티셔닝의 복잡성 없이 분산 처리를 달성하는 방법: **동일한 Job을 여러 인스턴스로 실행하되, 각각 다른 `JobParameters`로 처리 범위를 분할**
  - 파티셔닝의 `ExecutionContext`와 동일한 원리를 `JobParameters`로 적용하는 것
...
- 원격 파티셔닝이 정당화되는 유일한 경우: 분산된 Worker들의 `StepExecution`을 **하나의 `JobExecution`으로 묶어 추적·관리**해야 하는 요구가 있을 때
