...
- **kubelet**: 모든 노드에 상주하는 에이전트(데몬). 노드와 컨트롤 플레인을 잇는 핵심 컴포넌트
  - **PodSpec 수신** — API Server를 watch하며 자기 노드에 할당된 Pod 정의를 받음
  - **컨테이너 실행 위임** — CRI(Container Runtime Interface)로 컨테이너 런타임에 start/stop 명령 (직접 컨테이너를 실행하지 않음)
  - **상태 보고** — Pod phase, container state, 노드 리소스를 주기적으로 API Server에 보고 (heartbeat 포함)
  - **probe·hook 실행** — liveness/readiness/startup probe와 `postStart`/`preStop` hook을 직접 호출
  - "노드가 죽었다" = **kubelet의 heartbeat가 끊긴 상태**
